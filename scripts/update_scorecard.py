...
def grade_row(row, market, today_str):
    ...
    if row.get('prev_close_date') >= today_str:
        # Skip rows whose predictions were made today — those are for next week
        return False
    field = {'SPX': 'spx', 'NDX': 'ndx', 'QQQ': 'qqq'}[row['ticker']]
...
def append_predictions(rows, market, week_label, today_str):
    # Dedupe by (ticker, week_label) — applies to ALL existing rows, graded or not
